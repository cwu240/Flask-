...
def checkNumber():
    ...
    if request.method == "POST":
        ...
        if not number:
            return 'Error, please enter a number.<br><a href="/calculate">Home</a>'

        elif number.isalpha():
            return f'{number} is not a number. <br><a href="/calculate">Home</a>'

        elif int(number) % 2 == 0:
            return f'{number} is even. <br><a href="/calculate">Home</a>'

        else:
            return f'{number} is odd. <br><a href="/calculate">Home</a>'
...
def displayRegistrationPage():
    ...
    studentOrg = request.form['organ']

    # Append this value to studentOrganisationDetails
    # Display studentDetails.html with all students and organisations
    return render_template('StudentDetails.html', studentName=studentName, studentOrg=studentOrg)
